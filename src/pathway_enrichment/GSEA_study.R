...
ggplot(gsea_study, aes(x = idx, y = NES)) +
  geom_point(aes(size = -log10(padj)), alpha = 0.5, colour = "red2") +
  ...
  geom_text_repel(
    ...
    direction = "x",
    hjust = 1,
    xlim = c(min(gsea_study$idx) +1, min(gsea_study$idx) + 35),
    segment.size = 0.4,
    segment.alpha = 0.8,
    size = 3
  ) + 
  ...
  geom_text_repel(
    ...
    direction = "x",
    hjust = 0,
    xlim = c(max(gsea_study$idx) - 0.5, max(gsea_study$idx) -40),
    segment.size = 0.4,
    segment.alpha = 0.8,
    size = 3
  ) + 
